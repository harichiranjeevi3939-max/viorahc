...
import type { UserAnswer, QuizPayload, MCQ } from '../../types';
...
interface QuizResultsViewProps {
    ...
    theme: Theme;
    mcqs: MCQ[];
    quizContext: { content: string; difficulty: 'Basic' | 'Standard' | 'Hard' } | null;
    onShare: (payload: QuizPayload, type: 'quiz') => void;
    activeGroupId: string | null;
}

const QuizResultsView: React.FC<QuizResultsViewProps> = ({ results, onRetry, onExit, onExplainRequest, theme, mcqs, quizContext, onShare, activeGroupId }) => {
    ...
    const handleShare = () => {
        ...
        const payload: QuizPayload = {
            topic: quizContext.content.substring(0, 50) + '...', // Simple topic for now
            mcqs: mcqs,
            difficulty: quizContext.difficulty,
        };
        onShare(payload, 'quiz');
    };
        ...
        <div className="flex-1 overflow-y-auto p-6 w-full max-w-4xl mx-auto animate-slide-in">
            ...
            <div className="flex justify-center mt-6 space-x-4">
                 ...
                 <button 
                    ...
                    className={`flex items-center gap-2 px-4 py-2 text-white rounded-lg transition-opacity hover:opacity-90 disabled:opacity-50 disabled:cursor-not-allowed ${theme === 'professional' ? 'bg-sky-500' : 'bg-indigo-500'}`}
                    title={activeGroupId ? "Share results with your group" : "Join a group to share"}
                 >
